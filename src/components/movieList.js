...
function MovieList({ movies }) {
      ...
      <div key={index} className="mb-6 md:mb-0 md:w-full">
        ...
        <div className="mt-2 flex flex-col">
          ...
          <div className="text-sm flex items-center mt-1">
            ...
            <div className="text-orange-400 ml-2"> {movie.vote_average} / 10</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto flex flex-col justify-baseline">
      <h2 className="text-5xl font-bold my-4">Popular Movies</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-5 md:gap-6 lg:gap-8">
        {movie}
      </div>
    </div>
  );
